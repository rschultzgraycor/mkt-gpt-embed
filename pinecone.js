...
async function processPendingFiles(pool, client) {
  ...
  //   .query("SELECT * FROM fileSync WHERE agentId = 3 AND isDeleted = 0 AND ignoreFile = 0 AND syncStatus = 'pending'");

  const results = await pool.request()
    .query("SELECT * FROM fileSync WHERE agentId = 3 AND isDeleted = 0 AND ignoreFile = 0 AND (syncStatus = 'pending' OR syncStatus = 'error_download' OR syncStatus = 'error_chunkLengthexceeded')");
...
async function reprocessUpdatedFiles(pool, client) {
  const results = await pool.request().query(`
    SELECT * FROM fileSync WHERE agentId = 3 AND isDeleted = 0 AND ignoreFile = 0 AND syncStatus = 'updated'
  `);
...
async function removeDeletedFiles(pool) {
  const results = await pool.request()
    .query("SELECT graphid, chunkCount FROM fileSync WHERE agentId = 3 AND (isDeleted = 1 or ignoreFile = 1) AND chunkCount IS NOT NULL AND chunkCount > 0");
